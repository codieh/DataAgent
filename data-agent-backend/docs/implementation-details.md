...
- 提示词参考了 `management` 的写法，强调：
  - **宁放过，不杀错**
  - 多轮追问默认偏向 `DATA_ANALYSIS`
  - 只有明确闲聊/无关输入才拦截
- 解析结果时还会兼容 `management` 风格标签：
  - `《可能的数据分析请求》`
  - `《闲聊或无关指令》`
...
如果命中安全策略，不直接抛异常，而是写入：

- `resultMode=blocked_sensitive_sql`
- `resultMode=blocked_wide_export`
