...
### 13.1 优先完善 Recall（新增规划：2026-03-31）

目标：

- 让 evidence / schema 的召回更稳定、更可控、更接近 management 的实现方式

计划拆分：

- 继续细化 evidence metadata 过滤
- 强化 schema “先表、再列”的召回链路
- 优化 `keyword / vector / hybrid` 的融合策略
- 做 recall 诊断基线与效果对比
...
- 减少不相关 evidence 污染
- 提高 schema 命中率
- 让 hybrid recall 的提升可以被量化证明

### 13.2 接入 Document RAG（新增规划：2026-03-31）

目标：

- 将本地业务文档也纳入统一 recall 来源，形成 `Schema + Evidence + Document` 的多源召回系统

计划拆分：

- 新增 `DOCUMENT` 类型
- 支持本地 `md/txt/json` 文档读取
- 对文档做 chunk 切分并转成 `RecallDocument`
- 建立文档索引初始化与状态查看
- 将 document recall 接入主链路，先作为 evidence 的辅助来源

希望达到的效果：

- 补充 schema 无法表达的业务定义与规则
- 降低业务口径类问题的幻觉风险
- 让项目更像完整的 Agent / RAG 系统

### 13.2.1 Document RAG 当前暴露出的优化项（记录时间：2026-03-31 21:00）

基于“查询高消费用户”这类冒烟测试，当前已经确认 Document RAG 链路打通，但还存在以下待优化点：

- **document 召回相关性不足**
  - 现象：命中了 `user-segments`，但同时也命中了 `inventory-and-sales` 这类偏题文档块
  - 影响：document 上下文中混入噪声，可能干扰后续 SQL 生成

- **document / evidence 的语义边界还不够清晰**
  - 现象：用户问“高消费用户”，evidence 命中了“核心用户定义”
  - 影响：系统容易把“高消费用户”和“核心用户”混在一起理解

- **schema recall 仍然偏宽**
  - 现象：本例中召回了 `products`，但最终 SQL 并未使用
  - 影响：prompt 中出现不必要 schema，可能增加 SQL 生成噪声

- **“高消费用户”这类概念缺少显式业务定义约束**
  - 现象：最终 SQL 只是按总消费金额排序用户，而不是基于明确阈值或文档定义筛选
  - 影响：结果“可用但不够严格”，更像排行榜而不是明确定义后的用户分层

后续优先优化方向建议：

- 优先增强 document metadata/topic 过滤
- 优先优化 document/evidence 的融合与优先级
- 再考虑对“用户分层类问题”增加更明确的业务定义使用策略

### 13.2.2 多查询输入的当前限制（记录时间：2026-04-02）

当前项目对**单一数据分析问题**已经可以稳定处理，但对于一条输入中同时包含多个截然不同的查询请求，还不具备稳定支持能力。

典型例子：

- “查销量最高的商品，并统计每个分类的销售额”
- “列出高消费用户，再查低库存商品”
- “统计最近一个月订单数，再看退款订单占比”
...
- 当前主链路仍然默认是：
  - `一个 query -> 一次 intent -> 一次 enhance -> 一次 SQL generate`
- `ENHANCE` 仍然主要服务单一中心问题
- `SQL_GENERATE` 默认目标仍是一条 SQL
- `RESULT` 也是围绕单个结果集做总结
- 系统尚未具备：
  - query decomposition（查询拆解）
  - sub-task planning（子任务规划）
  - multi-query orchestration（多子任务编排）
...
- 增加多查询识别与拆分
- 将一个输入拆成多个 sub-query
- 每个 sub-query 单独走一遍 pipeline / graph 子流程
- 最后聚合多个结果并统一总结

这部分和后续 Graph 编排升级高度相关，适合作为中后期增强项继续推进。
