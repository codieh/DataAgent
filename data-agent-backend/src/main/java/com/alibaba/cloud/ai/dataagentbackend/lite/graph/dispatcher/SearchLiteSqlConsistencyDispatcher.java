...
@Component
public class SearchLiteSqlConsistencyDispatcher implements EdgeAction {

	private static final Logger log = LoggerFactory.getLogger(SearchLiteSqlConsistencyDispatcher.class);
	...
	public String apply(OverAllState state) {
		...
		if (isUsableSql(sql)) {
			log.info("graph sql-consistency dispatcher: sqlLen={}, route to {}", sql.length(), SQL_EXECUTE_NODE);
			return SQL_EXECUTE_NODE;
		}

		log.info("graph sql-consistency dispatcher: empty or invalid sql, route to {}", PREPARE_RESULT_NODE);
